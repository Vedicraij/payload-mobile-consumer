...
export const CTABlock = ({block, onNavigate}: {block: BlockOf<'restaurantCTA'>; onNavigate: (path: string) => void}) => <View style={styles.cta}><Text style={styles.ctaHeading}>{block.headline}</Text><Text style={styles.ctaBody}>{block.description}</Text>{block.label ? <Pressable accessibilityRole="button" onPress={() => openContentLink(block.destination?.path || block.href, onNavigate)} style={styles.ctaButton}><Text style={styles.ctaButtonText}>{block.label}</Text></Pressable> : null}</View>;
